...
mat4 create_look_at(vec3 from, vec3 to) {
    ...
    if (fabs(dot(f, global_up)) > 0.999f) {
        global_up = { 0.0f, 0.0f, 1.0f };
    }
...
vec3 normalize(vec3 v) {
    ...
    if (magnitude == 0) {
        std::cerr << "WARNING: Attempting to normalize a zero-length vector!" << std::endl;
        return { 1.0f, 0.0f, 0.0f }; // Default to a unit vector
    }
